...
string aggFunc = "count";					//aggragate function name:count sum only now
string table1Name ;		//name of the first table
string table2Name ;		//name of the second table
int k = 10;								//the number of explanations you want to get
...
int main(int argc,char* argv[]) {
	...
	if( order_int > focusALL.size() ){
		cout << "[Error   ] argv order too big " << endl;
		return -1;
	}	
	...
	//trendiff.table->printTimeMap();
	int ltime = 0;
	int rtime = trendiff.table->max_time ;
	cout << "maxtime: " << rtime << endl;
	trendiff.computeD0(ltime,rtime);
	trendiff.splitColumns();
	trendiff.cacheAllLeaf(ltime, rtime);
	trendiff.cubeAllExps();
	time_t t3 = GetCurrentTimeMsec();
	
	//trendiff.printAllExpValues();
	trendiff.FindTopkExplanations(k, ltime, rtime);
	time_t t4 = GetCurrentTimeMsec();

	trendiff.HigeOrderCompute(ltime, rtime);
	time_t t5 = GetCurrentTimeMsec();
	...
	cout << "[TimeCost] compute proj and sum for all exps: " << (t3 - t2) << "ms" << endl;
	cout << "[TimeCost] compute actual score of exps: " << (t4 - t3) << "ms" << endl;
	cout << "[TimeCost] high exps: " << (t5 - t4) << "ms" << endl;
	cout << "[TimeCost] total time cost: " << (t5 - t1) << "ms" << endl;

	cout << "[LOGGING ] PATH : " << path << " k: " <<  k  << " order :" << order_int << endl;	
	cout << "=================================" << endl;
}
