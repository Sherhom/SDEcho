# [baseline] Use BOExplain for Sequential Explanation Queries

Next, we will introduce how to use `BOExplain` (baseline) for sequential explanation queries.
...
so that the `BOExplain` program can correctly connect to the database and read the data.
...
so that the `BOExplain` program can correctly use the interfaces provided by PostgreSQL.


##  Step 2: Compile the BOExplain Program
...
##  Step 3: Use BOExplain for Sequential Explanation Queries
...
###  3.2 Using BOExplain for Sequential Explanation Queries
After compiling the program in the `build` directory,
you can execute the following command:
```
./BOExplain TPCH_S01/1.txt
```
This indicates that you will use BOExplain to complete the explanation query configured in `TPCH_S01/1.txt`.
