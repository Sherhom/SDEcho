...
public class APrioriLinear {
    private final int trendDiffShift = 2;
    ...
    public List<APLExplanationResult> explain(
        ...
        boolean isTrendDiff = (timeLine != null);
        int maxTime = 0;
        if(isTrendDiff){
            for(double t: timeLine){
                maxTime = Math.max(maxTime, (int)t);
            }
        }
        ...
        if(isTrendDiff){
            double[] globalAggregates = new double[maxTime + 1 + trendDiffShift];
            Arrays.fill(globalAggregates, 0);
            for(int i = 0 ; i < numRows; ++i){
                if(aggregateColumns[0][i] > 0.5){
                    globalAggregates[(int) timeLine[i] + trendDiffShift] ++;
                    globalAggregates[0] += 1;
                }else{
                    globalAggregates[(int) timeLine[i] + trendDiffShift] --;
                    globalAggregates[1] += 1;
                }
            }
            qualityMetrics[0].initialize(globalAggregates);
        }else{
        ...
        final double[][] aRows;
        if(!isTrendDiff) {
            aRows = new double[numRows][numAggregates];
            for (int i = 0; i < numRows; i++) {
                for (int j = 0; j < numAggregates; j++) {
            ...
            aRows = new double[numRows][maxTime + 1 + trendDiffShift];
            for (int i = 0; i < numRows; i++) {
                Arrays.fill(aRows[i], 0.0);
                if(aggregateColumns[0][i] > 0.5){
                    aRows[i][0] = 1;
                    aRows[i][(int)timeLine[i] + trendDiffShift] = 1;
                }else{
                    aRows[i][1] = 1;
                    aRows[i][(int)timeLine[i] + trendDiffShift] = -1;
                }
        ...
        for (int curOrder = 1; curOrder <= maxOrder; curOrder++) {
            ...
            for (int i = 0; i < numThreads; i++) {
                threadSetAggregates.add(new FastFixedHashTable(cardinality, isTrendDiff? maxTime + 1 + trendDiffShift: numAggregates, useIntSetAsArray));
            }
                        ...
                        for (int colNum = 0; colNum < numColumns; colNum++) {
                            ...
                            if (colCardinalities[colNum] < AttributeEncoder.cardinalityThreshold) {
                                for (Integer curOutlierCandidate : outlierList[colNum]) {
                                    ...
                                    double[] series = new double[]{outlierCount, outlierCount + inlierCount};

                                    if(isTrendDiff){
                                        double[] trendDiff = new double[finalMaxTime + 3];
                                        Arrays.fill(trendDiff, 0);
                                        for(int i = 0 ; i < numRows; ++i){
                                            if(curCandidate.getFirst() == attributes[colNum][i]){
                                                if(aggregateColumns[0][i] > 0){
                                                    trendDiff[(int) timeLine[i] + 2] ++;
                                                }else{
                                                    trendDiff[(int) timeLine[i] + 2] --;
                                                }
                                            }
                                        }
                                        trendDiff[0] = series[0];
                                        trendDiff[1] = series[1];
                                        series = trendDiff;
                                    }

                                    updateAggregates(thisThreadSetAggregates, curCandidate, aggregationOps,
                                            series, numAggregates);
                                }
                            } else {
                                ...
                                for (int rowNum = startIndex; rowNum < endIndex; rowNum++) {
                                    ...
                                    if(isTrendDiff){
                                        updateAggregatesForTrendDiff(thisThreadSetAggregates, curCandidate, aggregationOps,
                                                aRows[rowNum], finalMaxTime + 1 + trendDiffShift);
                                    }else{
                                        updateAggregates(thisThreadSetAggregates, curCandidate, aggregationOps,
                        ...
                        for (int colNumOne = 0; colNumOne < numColumns; colNumOne++) {
                            ...
                            for (int colNumTwo = colNumOne + 1; colNumTwo < numColumns; colNumTwo++) {
                                ...
                                if (colCardinalities[colNumOne] < AttributeEncoder.cardinalityThreshold &&
                                        colCardinalities[colNumTwo] < AttributeEncoder.cardinalityThreshold &&
                                    ...
                                    if(!isTrendDiff){
                                        ...
                                        allTwoBitmap(thisThreadSetAggregates, outlierList, aggregationOps, singleNextArray,
                                                byThreadBitmap[curThreadNum], colNumOne, colNumTwo, useIntSetAsArray,
                                                curCandidate, numAggregates, timeLine, attributes, finalMaxTime, aggregateColumns[0]);
                                    }
                                }  else {
                                    if(!isTrendDiff){
                                        ...
                                        allTwoNormalForTrendDiff(thisThreadSetAggregates, curColumnOneAttributes,
                                                curColumnTwoAttributes, aggregationOps, singleNextArray,
                                                startIndex, endIndex, useIntSetAsArray, curCandidate, aRows,
                                                finalMaxTime + 1 + trendDiffShift);
                                    }
                                }
                            }
                        }
                                    ...
                                    if (colCardinalities[colNumOne] < AttributeEncoder.cardinalityThreshold &&
                                        ...
                                        if(!isTrendDiff){
                                            ...
                                            allThreeBitmap(thisThreadSetAggregates, outlierList, aggregationOps,
                                                    singleNextArray, byThreadBitmap[curThreadNum],
                                                    colNumOne, colNumTwo, colNumThree, useIntSetAsArray, curCandidate,
                                                    numAggregates, timeLine, attributes, finalMaxTime, aggregateColumns[0]);
                                        }
                                        ...
                                        if(!isTrendDiff){
                                            ...
                                            allThreeNormalForTrendDiff(thisThreadSetAggregates, curColumnOneAttributes,
                                                    curColumnTwoAttributes, curColumnThreeAttributes,
                                                    aggregationOps, singleNextArray, startIndex, endIndex,
                                                    useIntSetAsArray, curCandidate, aRows, finalMaxTime + 1 + trendDiffShift);
                                        }
                                    }
                                }
                            }
                ...
                if (useIntSetAsArray) {
                    for (IntSet curCandidateKey : set.keySet()) {
                        ...
                        if (candidateVal == null) {
                            setAggregates.put(curCandidateKey, Arrays.copyOf(curCandidateValue, isTrendDiff? maxTime + 1 + trendDiffShift : numAggregates));
                        } else {
                            if(isTrendDiff){
                                candidateVal = AggregationOp.SeriesSum.combineDoubles(candidateVal, curCandidateValue);
                            }else{
                    ...
                    for (long curCandidateKeyLong : set.keySetLong()) {
                        ...
                        if (candidateVal == null) {
                            ...
                            if(isTrendDiff){
                                AggregationOp.SeriesSum.combineDoubles(candidateVal, candidateVal);
                            }else{
                                for (int a = 0; a < numAggregates; a++) {
                                    AggregationOp curOp = aggregationOps[a];
                                    candidateVal[a] = curOp.combine(candidateVal[a], curCandidateValue[a]);
