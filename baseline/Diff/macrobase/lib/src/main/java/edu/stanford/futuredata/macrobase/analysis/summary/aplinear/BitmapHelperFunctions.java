...
public class BitmapHelperFunctions {
    ...
    public static void updateAggregatesForTrendDiff(FastFixedHashTable thisThreadSetAggregates,
                                        IntSet curCandidate, AggregationOp[] aggregationOps,
                                        double[] aggregateVal, int serLen) {
        double[] candidateVal = thisThreadSetAggregates.get(curCandidate);
        if (candidateVal == null) {
            thisThreadSetAggregates.put(curCandidate,
                    Arrays.copyOf(aggregateVal, serLen));
        } else {
            AggregationOp curOp = AggregationOp.SeriesSum;
            candidateVal = curOp.combineDoubles(candidateVal, aggregateVal);
            thisThreadSetAggregates.set(curCandidate, candidateVal);
        }
    }
    ...
    public static void allTwoNormalForTrendDiff(FastFixedHashTable thisThreadSetAggregates,
                                    int[] curColumnOneAttributes, int[] curColumnTwoAttributes,
                                    AggregationOp[] aggregationOps, boolean[] singleNextArray,
                                    int startIndex, int endIndex,
                                    boolean useIntSetAsArray, IntSet curCandidate,
        ...
        for (int rowNum = startIndex; rowNum < endIndex; rowNum++) {
            ...
            updateAggregatesForTrendDiff(thisThreadSetAggregates, curCandidate, aggregationOps, aRows[rowNum], numAggregates);
        }
    }
    ...
    public static void allTwoBitmap(FastFixedHashTable thisThreadSetAggregates,
                                    ...
                                    int colNumOne, int colNumTwo,
                                    boolean useIntSetAsArray, IntSet curCandidate, int numAggregates,
                                    double[] timeLine, final int[][] attributes, int finalMaxTime, double[] isOutlier) {
        for (Integer curCandidateOne : outlierList[colNumOne]) {
            if (curCandidateOne == AttributeEncoder.noSupport || !singleNextArray[curCandidateOne])
                continue;
            for (Integer curCandidateTwo : outlierList[colNumTwo]) {
                ...
                if(timeLine!=null){
                    ...
                    for(int i = 0 ; i < attributes.length; ++i){
                        if(attributes[colNumOne][i] == curCandidate.getFirst()
                                && attributes[colNumTwo][i] == curCandidate.getSecond()){
                            if(isOutlier[i] > 0){
    ...
    public static void allThreeNormalForTrendDiff(FastFixedHashTable thisThreadSetAggregates,
                                      int[] curColumnOneAttributes, int[] curColumnTwoAttributes,
                                      int[] curColumnThreeAttributes,
                                      AggregationOp[]  aggregationOps, boolean[] singleNextArray,
                                      int startIndex, int endIndex,
                ...
                for (Integer curCandidateThree : outlierList[colNumThree]) {
                    ...
                    if(timeLine!=null){
                        ...
                        for(int i = 0 ; i < attributes.length; ++i){
                            if(attributes[colNumOne][i] == curCandidate.getFirst()
                                    && attributes[colNumTwo][i] == curCandidate.getSecond()
                                    && attributes[colNumThree][i] == curCandidate.getThird()){
                                if(isOutlier[i] > 0){
                                    trendDiff[(int) timeLine[i] + 2] ++;
                                }else{
                                    trendDiff[(int) timeLine[i] + 2] --;
