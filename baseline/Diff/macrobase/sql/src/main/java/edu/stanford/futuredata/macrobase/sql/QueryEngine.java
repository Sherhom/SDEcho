...
import edu.stanford.futuredata.macrobase.analysis.summary.aplinear.APLTrendDiffSummarizer;
...
class QueryEngine {
    ...
    DataFrame executeQuery(QueryBody query) throws MacroBaseException {
        if (query instanceof QuerySpecification) {
            ...
            DataFrame dataFrame = executeDiffQuerySpec(diffQuery);
            long endTime=System.currentTimeMillis();
            System.out.println("================================================");
            System.out.println("==== [RESULT TIME] TOTAL COST TIME : " + (endTime - startTime) / 1000.0 + " s ====");
            System.out.println("================================================");
            return dataFrame;
        }
    ...
    private DataFrame executeDiffQuerySpec(final DiffQuerySpecification diffQuery)
        ...
        final String orderColName = "timeline";
        ...
        if (diffQuery.hasTwoArgs()) {
            ...
            if (matchesDiffJoinCriteria(first.getQuery().getQueryBody(),
                ...
                orderNameList.add(orderColName);
                timeLine = (common.getDoubleColsByName(orderNameList)).get(0);
            } else {
                // execute subqueries
                ...
                timeLine = dfToExplain.getDoubleColumnByName(orderColName);
            }
        } else {
        ...
        // execute diff
        APLSummarizer summarizer;
        if(diffQuery.isTrendDiff()){
            summarizer = new APLTrendDiffSummarizer(diffQuery.getMinTrendDiffExpression().getMinTrendDiffScore(), timeLine);
            ((APLTrendDiffSummarizer) summarizer).setThreshold(diffQuery.getMinTrendDiffExpression().getMinTrendDiffScore());
        }else {
            summarizer = new APLOutlierSummarizer(true);
        }
