...
public:
	...
	void AbssumOfRightOrderLeaf(int ltime,int rtime);

	void cubeAllAbssum();

	bool isRecordOfExpRight(int index, vector<int>& exp_right);

	void projOfRightOrderLeaf(vector<int>& exp_right, int ltime, int rtime);

	void cubeAllproj();

	void computeDeeperExps(vector<int>& exp_right, int ltime, int rtime);

	void dimensionPrune(int ltime, int rtime);

	//conside more cols (right order)
	void HigeOrderCompute(int ltime, int rtime);
