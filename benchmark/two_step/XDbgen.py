...
# 生成包含噪声的序列
class DBgen:
    def __init__(self, sequence:Sequence, sql_path):
        # 连接三个数据库
        self.conn_db = Connection('xdata')
        self.conn_noise = Connection('xdata_noise')
        self.conn_origin = Connection('xdata_ori')
        self.sequence = sequence
        self.noise_file = os.path.join(sql_path, 'noise.json')
    ...
    def delete_noise(self):
        data = pd.read_csv(os.path.join(self.sql_path, f'data.csv'))
        table = self.sequence.table
        self.conn_db.execute_sql(f'drop table if exists {table}; commit')
        engine = create_engine(f'postgresql+psycopg2://yf:qwerpoiu@127.0.0.1:5438/xdata')
        pd.io.sql.to_sql(data,f'{table}',engine,schema='public')
        self.conn_db.execute_sql(f'alter table {table} drop column index')
    ...
    def gen(self, k=3):
        ...
        # pbar = tqdm(pattern_values[0:-1:int(len(pattern_values)/100)])
        ...
        # 修改数据库中数据
        # for noise in heap:
        #     self.add_noise(noise.pattern_value)
        print("完成增加噪声")
        self.conn_noise.execute_sql("drop view temp_noise")
        self.conn_db.execute_sql("commit")
    
    def encode(self, db, conn: Connection):
        csv_path = os.path.join(self.sql_path, 'raw_data.csv')
        dict_path = os.path.join(self.sql_path, 'dict.json')

        table = 'flat_table'
        fields = conn.desc_table(table)
        print('save database')
        with open(csv_path, 'w+') as file:
            writer=csv.writer(file)
            writer.writerow(fields)
            for value in np.array(conn.execute_sql(f'select * from {table}'),str):
                writer.writerow(value)
        
        print('encode')
        with open(os.path.join(self.sql_path, f'raw_data.csv'),"r") as read_file:
            ...
            if os.path.exists(dict_path):
                ...
                for col_name in header:
                    dict.update({col_name:[]})

            with open(os.path.join(self.sql_path, f'data.csv'),"w",newline="") as write_file:
                writer=csv.writer(write_file)
                writer.writerow(header)
                for row in reader:
                    for index, item in enumerate(row):
                        if item is not None and not bool(re.match(r'^[+-]?(\d+|\d*\.\d+)$', item)):
                            col_name=header[index]
                            list = dict[col_name]
                            if item not in list:
                                list.append(item)
                            # print(list.index(item))
                            row[index]=list.index(item)
                    writer.writerow(row)
            # print(dict)               

        with open(dict_path,"w") as dict_file:
            json.dump(dict,dict_file,indent=2)

        print('import database')
        data = pd.read_csv(os.path.join(self.sql_path, f'data.csv'))
        conn.execute_sql(f'drop table if exists {table}; commit')
        engine = create_engine(f'postgresql+psycopg2://yf:qwerpoiu@127.0.0.1:5438/{db}')
        pd.io.sql.to_sql(data,f'{table}',engine,schema='public')
        conn.execute_sql(f'alter table {table} drop column index')
        conn.execute_sql('commit')

        os.remove(os.path.join(self.sql_path, f'raw_data.csv'))
        # os.remove(os.path.join(self.sql_path, f'data.csv'))

    def encode_all(self):
        print('encode data...')
        database = ['xdata', 'xdata_ori', 'xdata_noise']

        with open(os.path.join(self.sql_path, 'join.sql'),'r',encoding='utf-16') as f:
            join_sql = f.read()
            self.conn_db.execute_sql(join_sql)
            self.conn_noise.execute_sql(join_sql)
            self.conn_origin.execute_sql(join_sql)
        
        self.encode('xdata_noise', self.conn_noise)
        self.encode('xdata', self.conn_db)
        self.encode('xdata_ori', self.conn_origin)
...
if __name__=='__main__':
    ...
    dbgen = DBgen(Sequence(os.path.join(sql_path,'sql.json')), sql_path)
    dbgen.encode_all()
    noises = dbgen.gen(10)